...
class Informations(commands.Cog):
    ...
    async def info_bot(self, context):
        ...
        print("Info ... TODO")
        try:
            embed = get_bot_info_embed(self.settings.embeds["bot_info"],
                                       self.settings.config["bot_description"],
                                       self.settings.config["langages"],
                                       self.settings.config["version"],
    ...
    async def info(self, context, user: discord.Member):
        ...
        try:
            ...
            embed = get_user_info_embed(self.settings.embeds["user_info"],
    ...
    async def server_info(self, context):
        ...
        try:
            ...
            embed = get_server_info_embed(self.settings.embeds["server_info"],
                                          server, description,
                                          self.bot.user.avatar_url)
